...
private float TryGetCairoFontPx(CairoFont font)
{
    ...
    return -1;
        ...
        public int MaxRenderDistance
        {
            get
            {
                return MaxMaxRenderDistanceXZ;
            }
        }

        /// <summary>Compatibility shim: slider upper bound is now hard-coded to 20000.</summary>
        public void SetMaxRenderDistance(int blocks)
        {
            SetMaxRenderDistanceXZ(blocks);
        }
        ...
        public int MaxRenderDistanceXZ
        {
            get
            {
                int val = clientConfig?.renderDistance ?? 10000;
        ...
        public override void StartClientSide(ICoreClientAPI api)
        {
            ...
            // Normalize legacy config values that may come from older versions
            if (clientConfig == null) clientConfig = new WaypointBeaconClientConfig();
            if (clientConfig.renderDistance < MinRenderDistance) clientConfig.renderDistance = MinRenderDistance;
            if (clientConfig.renderDistance > MaxRenderDistance) clientConfig.renderDistance = MaxRenderDistance;
